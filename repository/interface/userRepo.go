...
type UserRepository interface {
	...
	FindUser(email string, userRole string) (domain.User, error)

	ListProducts(page, perPage int) ([]domain.ProductResponse, utils.MetaData, error)
	ViewProduct(id string) (domain.Products, error)
	//Wishlist
	...
	//Cart
	AddCart(user_id string, product_id string) (error, string)
